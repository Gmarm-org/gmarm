...
-- Corregir caracteres especiales que pueden corromperse durante la inserci�n
...
-- Agregar constraint de foreign key despu�s de crear tipo_proceso (si no existe)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Tabla de relaci�n tipo_cliente-importacion
-- NOTA: Esta tabla es necesaria para la relaci�n @ManyToMany en TipoCliente.java
...
CREATE TABLE IF NOT EXISTS cliente (
    ...
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- Campos para proceso de aprobaci�n del jefe de ventas
    ...
    canton VARCHAR(100),
    -- Informaci�n de representante legal (para empresas)
    representante_legal VARCHAR(100),
    -- Informaci�n de empresa (solo para tipo empresa)
    ruc VARCHAR(13),
    ...
    -- Informaci�n militar (solo para uniformados - militares y polic�as, NULL para otros tipos)
...
COMMENT ON COLUMN cliente.email_verificado IS 
    'Indica si el correo electr�nico del cliente ha sido verificado mediante el enlace enviado por email';
...
CREATE TABLE IF NOT EXISTS tipo_documento (
    ...
    tipo_proceso_id BIGINT REFERENCES tipo_proceso(id), -- NULL si es para grupos de importaci�n
    estado BOOLEAN DEFAULT true,
...
COMMENT ON COLUMN tipo_documento.grupos_importacion IS 
    'true si es para grupos de importaci�n (tipo_proceso_id debe ser NULL), false si es para clientes (puede tener tipo_proceso_id o NULL para todos)';
...
-- Tabla de categor�as de armas (estructura simplificada)
...
-- Tabla de im�genes de armas (m�ltiples im�genes por arma)
...
-- эndice para mejorar consultas de im�genes por arma
...
-- Tabla de n�meros de serie de armas
-- Esta tabla almacena todos los n�meros de serie �nicos de armas f�sicas
...
CREATE TABLE IF NOT EXISTS arma_serie (
    id BIGSERIAL PRIMARY KEY,
    numero_serie VARCHAR(100) UNIQUE NOT NULL,  -- N�mero de serie �nico del arma f�sica
    ...
    cliente_arma_id BIGINT,  -- Relaci�n con la reserva del cliente (FK se agrega despu�s)
    usuario_asignador_id BIGINT REFERENCES usuario(id),  -- Usuario que asign� la serie
    lote VARCHAR(50),  -- Lote o grupo de importaci�n (opcional, deprecated - usar grupo_importacion_id)
    grupo_importacion_id BIGINT,  -- Grupo de importaci�n al que pertenece esta serie (FK se agrega despu�s)
    licencia_id BIGINT,  -- Licencia asociada al grupo de importaci�n (FK se agrega despu�s)
...
-- эndices para optimizar b�squedas (sin cliente_arma_id por ahora)
...
-- Tabla de relaci�n cliente-arma (cuando un cliente elige)
...
-- Tabla de relaci�n cliente-accesorio (cuando un cliente elige)
...
CREATE TABLE IF NOT EXISTS cuota_pago (
    ...
    numero_recibo VARCHAR(100), -- N�mero de recibo (reemplaza "N�mero de Comprobante")
...
-- Los tipos de cliente ya tienen relaci�n directa con tipo_proceso
...
-- Tabla de grupos de importaci�n
CREATE TABLE IF NOT EXISTS grupo_importacion (
    ...
    tra VARCHAR(20) UNIQUE, -- TRA-XXXXXXXXXX (formato: TRA- seguido de n�meros)
    ...
    numero_previa_importacion VARCHAR(100), -- N�mero de previa importaci�n
    observaciones TEXT,
    usuario_actualizador_id BIGINT REFERENCES usuario(id),
...
COMMENT ON COLUMN grupo_importacion.numero_previa_importacion IS 
    'N�mero de previa importaci�n';

-- Tabla de relaci�n cliente-grupo de importaci�n
...
-- Tabla de cupos por tipo de cliente en grupo de importaci�n
CREATE TABLE IF NOT EXISTS grupo_importacion_cupo (
    id BIGSERIAL PRIMARY KEY,
    grupo_importacion_id BIGINT NOT NULL REFERENCES grupo_importacion(id) ON DELETE CASCADE,
    licencia_id BIGINT NOT NULL REFERENCES licencia(id),
    tipo_cliente VARCHAR(20) NOT NULL, -- CIVIL, MILITAR, EMPRESA, DEPORTISTA
    cupo_consumido INTEGER NOT NULL, -- Cu�nto consume este grupo (25 civiles, 28 uniformados, etc.)
    cupo_disponible_licencia INTEGER NOT NULL, -- Cu�nto queda disponible en la licencia
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- Tabla de relaci�n grupo de importaci�n - vendedores (muchos a muchos)
CREATE TABLE IF NOT EXISTS grupo_importacion_vendedor (
    id BIGSERIAL PRIMARY KEY,
    grupo_importacion_id BIGINT NOT NULL REFERENCES grupo_importacion(id) ON DELETE CASCADE,
    vendedor_id BIGINT NOT NULL REFERENCES usuario(id) ON DELETE CASCADE,
    limite_armas INTEGER DEFAULT 0, -- L�mite de armas por vendedor en este grupo
    fecha_asignacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(grupo_importacion_id, vendedor_id)
);

-- Tabla de l�mites por categor�a para grupos de tipo CUPO
CREATE TABLE IF NOT EXISTS grupo_importacion_limite_categoria (
    ...
    limite_maximo INTEGER NOT NULL DEFAULT 0, -- L�mite m�ximo de armas de esta categor�a
    fecha_creacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    fecha_actualizacion TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(grupo_importacion_id, categoria_arma_id)
);

-- Tabla de documentos del grupo de importaci�n
...
-- Tabla de tokens de verificaci�n de correo electr�nico
...
-- Comentarios descriptivos para tabla de verificaci�n de email
COMMENT ON TABLE email_verification_token IS 
    'Tokens �nicos y temporales para verificaci�n de correo electr�nico de clientes. Expiran en 48 horas y son de un solo uso.';

COMMENT ON COLUMN email_verification_token.token IS 
    'Token UUID �nico generado para cada solicitud de verificaci�n';
...
COMMENT ON COLUMN email_verification_token.expires_at IS 
    'Fecha y hora de expiraci�n del token (48 horas desde la creaci�n)';
...
-- эndices para mejorar rendimiento
...
-- �ndice �nico para evitar que un cliente est� en m�ltiples grupos activos
...
-- =====================================================
-- 3. INSERCIѓN DE DATOS BASE (solo si no existen)
-- =====================================================
...
('VENDOR', 'Vendedor', 'Registro de clientes y selecci�n de armas cat�logo', 'LIBRE', true),
('SALES_CHIEF', 'Jefe de Ventas', 'Aprobaci�n de solicitudes y creaci�n de grupos de importaci�n', 'FIJO', true),
('FINANCE', 'Finanzas', 'Gesti�n de pagos y facturaci�n', NULL, true),
('OPERATIONS', 'Operaciones', 'Gesti�n de importaci�n y documentaci�n', NULL, true)
...
('Cupo Civil', 'CUPO_CIV', 'Proceso para importaci�n por cupo civil', true),
('Extracupo Uniformado', 'EXC_MIL', 'Proceso para importaci�n por extracupo militar/policial', true),
('Extracupo Empresa', 'EXC_EMP', 'Proceso para importaci�n por extracupo compa��a de seguridad', true),
('Cupo Deportista', 'CUPO_DEP', 'Proceso para importaci�n por cupo civil deportista', true)
ON CONFLICT (codigo) DO NOTHING;
...
('Militar Fuerza A�rea', 'AER', 'Personal activo de fuerzas armadas a�reas', true, true, false, false, false, false, true, 2),
('Uniformado Policial', 'POL', 'Personal activo de fuerza policial', true, false, true, false, false, false, false, 2),
('Compa��a de Seguridad', 'EMP', 'Compa��a de seguridad privada', true, false, false, true, false, false, false, 3),
...
('C�dula de Identidad', 'CED', 'Documento de identificaci�n personal', true),
('RUC', 'RUC', 'Registro �nico de Contribuyentes', true)
ON CONFLICT (codigo) DO NOTHING;

-- Insertar tipos de importaci�n evitando duplicados
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM tipo_importacion) THEN
        ...
        ('Cupo Civil', 25, 'Importaci�n regular para personas naturales civiles', true),
        ('Extracupo Uniformado', 1000, 'Importaci�n especial para personal uniformado militar y policial', true),
        ('Extracupo Compania', 1000, 'Importaci�n especial para empresas de seguridad', true),
        ('Cupo Deportista', 1000, 'Importaci�n regular para deportistas', true);
    END IF;
END $$;

-- Insertar relaci�n tipos de importacion con tipo de cliente
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE tipo_proceso_id = 1) THEN
        ...
        ('Copia de c�dula', 'Copia legible de la c�dula de identidad', true, 1, true, NULL, false),
        ...
        ('Fiscal�a', 'Certificado de no tener procesos por robos/violencia/as esinatos', true, 1, true, 'https://www.fiscalia.gob.ec/accesibilidad/consulta-de-noticias-del-delito/', false),
    ...
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE tipo_proceso_id = 2) THEN
        ...
        ('Credencial militar/policial', 'Credencial vigente de instituci�n armada o policial', true, 2, true, NULL, false),
        ('Certificado de servicio activo', 'Certificado de servicio activo vigente', true, 2, true, NULL, false),
        ('Copia de c�dula', 'Copia legible de la c�dula de identidad', true, 2, true, NULL, false),
        ...
        ('Fiscal�a', 'Certificado de no tener procesos por robos/violencia/as esinatos', true, 2, true, 'https://www.fiscalia.gob.ec/accesibilidad/consulta-de-noticias-del-delito/', false),
    ...
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE tipo_proceso_id = 3) THEN
        ...
        ('C�dula del representante legal', 'C�dula del representante legal', true, 3, true, NULL, false),
    ...
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE tipo_proceso_id = 4) THEN
        ...
        ('C�dula en PDF y papeleta de votaci�n', 'C�dula de identidad en formato PDF y papeleta de votaci�n', true, 4, true, NULL, false),
        ...
        ('Documento del Club de Tiro que certifique estar al d�a en sus cuotas y afiliado al mismo', 'Documento del Club de Tiro que certifique que el deportista est� al d�a en sus cuotas y afiliado al mismo', true, 4, true, NULL, false),
        ...
        ('Fiscal�a', 'Certificado de no tener procesos por robos/violencia/as esinatos', true, 4, true, 'https://www.fiscalia.gob.ec/accesibilidad/consulta-de-noticias-del-delito/', false),
    ...
    -- Documentos para grupos de importaci�n (grupos_importacion = true, tipo_proceso_id = NULL)
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE grupos_importacion = true) THEN
        ...
        ('Proforma a fabrica para importacion', 'Proforma recibida de la f�brica para la importaci�n', true, true, true, NULL),
        ('Solicitar carta inspeccion de rastrillo', 'Carta de inspecci�n de rastrillo', true, true, true, NULL),
        ('Documento de resolucion de importacion', 'Resoluci�n de importaci�n', true, true, true, NULL),
        ('Pedido de previa importacion', 'Pedido de previa importaci�n', true, true, true, NULL),
        ('Retiro de Guia', 'Documento de retiro de gu�a', true, true, true, NULL),
        ('Liquidacion de importacion', 'Liquidaci�n de importaci�n', true, true, true, NULL),
    ...
    IF NOT EXISTS (SELECT 1 FROM tipo_documento WHERE nombre = 'Resoluci�n para migrar serie al cliente') THEN
        INSERT INTO tipo_documento (nombre, descripcion, obligatorio, estado, grupos_importacion, tipo_proceso_id) VALUES
        ('Resoluci�n para migrar serie al cliente', 
         'Resoluci�n que autoriza la migraci�n del n�mero de serie del arma al cliente. Documento requerido antes de la entrega.', 
         true, 
         true, 
         false,  -- Es para clientes, NO para grupos de importaci�n
         NULL),  -- Aplicable a todos los tipos de proceso
        ('Gu�a de libre tr�nsito', 
         'Gu�a que permite el libre tr�nsito del arma desde el punto de llegada hasta el cliente. Documento requerido para la entrega final.', 
         true, 
         true, 
         false,  -- Es para clientes, NO para grupos de importaci�n
         NULL);  -- Aplicable a todos los tipos de proceso
...
-- Preguntas para militares/polic�as (tipo_proceso_id = 2)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM preguntas WHERE tipo_proceso_id = 2) THEN
        ...
        (2, '�Ya tiene firma electr�nica habilitada?', true, 3, true, 'SI_NO'),
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM preguntas WHERE tipo_proceso_id = 3) THEN
        ...
        (3, '�Tiene autorizaci�n de tenencia de armas?', true, 3, true, 'SI_NO'),
        (3, '�La empresa est� registrada en el SRI?', true, 4, true, 'SI_NO'),
...
('SMTP_HOST', 'smtp.gmail.com', 'Servidor SMTP para env�o de emails', true),
...
('SMTP_PASSWORD', 'vxdacadmloeecjiz', 'Contrase�a de aplicaci�n SMTP (NO la contrase�a normal)', true),
('SMTP_AUTH', 'true', 'Requiere autenticaci�n SMTP (true/false)', true),
...
('CORREOS_RECIBO', '["joseluis@guerreromartinez.com", "valeria@gmarm.com"]', 'Lista de correos electr�nicos que reciben copia autom�tica de los recibos de pago. Se env�a adem�s del correo del cliente. Formato: JSON array o separado por comas. Ejemplo: ["email1@example.com", "email2@example.com"] o "email1@example.com, email2@example.com"', true),
('DIAS_VALIDEZ_DOCUMENTOS', '30', 'D�as de validez para documentos subidos', true),
('PORCENTAJE_ANTICIPO', '40', 'Porcentaje de anticipo requerido', true),
('IVA', '15', 'Porcentaje de IVA aplicable', true),
('EDAD_MINIMA_CLIENTE', '25', 'Edad m�nima para clientes', true),
('MAX_INTENTOS_LOGIN', '3', 'M�ximo intentos de login antes de bloquear', true),
('TIPOS_PAGO_VALIDOS', 'CONTADO,CUOTAS', 'Tipos de pago v�lidos en el sistema', true),
('MAX_CUOTAS_PERMITIDAS', '6', 'M�ximo n�mero de cuotas permitidas', true),
('MIN_MONTO_CUOTA', '100.00', 'Monto m�nimo por cuota', true),
('EXPOFERIA_ACTIVA', 'false', 'Indica si la expoferia est� activa y se deben mostrar solo armas de expoferia', true),
...
-- Cupos por defecto de licencias de importaci�n (valores constantes)
('CUPO_DEFAULT_CIVIL', '25', 'Cupo por defecto para clientes civiles en licencias de importaci�n', false),
('CUPO_DEFAULT_MILITAR', '1000', 'Cupo por defecto para clientes uniformados (militares) en licencias de importaci�n', false),
('CUPO_DEFAULT_EMPRESA', '1000', 'Cupo por defecto para empresas de seguridad en licencias de importaci�n', false),
('CUPO_DEFAULT_DEPORTISTA', '1000', 'Cupo por defecto para deportistas en licencias de importaci�n', false)
ON CONFLICT (clave) DO NOTHING;

-- =====================================================
-- 4. INSERCIѓN DE DATOS DE LOCALIZACIѓN
-- =====================================================
...
--    - monto_total: Cu�nto debe pagar en total
--    - tipo_pago: 'CONTADO' o 'CUOTAS'
--    - numero_cuotas: Cu�ntas cuotas tiene (1 para contado)
--    - monto_cuota: Monto de cada cuota
--    - monto_pagado: Cu�nto ya pag�
--    - monto_pendiente: Cu�nto le falta (calculado autom�ticamente)
...
--    - numero_cuota: N�mero de la cuota (1, 2, 3, 4, 5, 6)
...
--    - cupo_civil: M�ximo 25 armas para clientes civiles
--    - cupo_militar: M�ximo 1000 armas para uniformados
--    - cupo_empresa: M�ximo 1000 armas para empresas de seguridad
--    - cupo_deportista: M�ximo 1000 armas para deportistas
...
--    - cupo_consumido: Cu�nto consume este grupo espec�fico
--    - cupo_disponible_licencia: Cu�nto queda disponible en la licencia
-- 
-- FLUJO DE TRABAJO:
-- 1. Se crea un grupo de importaci�n
...
('Gir�n', 'GIRON', true, (SELECT id FROM provincia WHERE codigo = 'AZU')),
...
('Nab�n', 'NABO', true, (SELECT id FROM provincia WHERE codigo = 'AZU')),
('O�a', 'ONA', true, (SELECT id FROM provincia WHERE codigo = 'AZU')),
...
('Camilo Ponce Enr�quez', 'CPEN', true, (SELECT id FROM provincia WHERE codigo = 'AZU'))
ON CONFLICT DO NOTHING;
...
('Echeand�a', 'ECHE', true, (SELECT id FROM provincia WHERE codigo = 'BOL')),
...
('Ca�ar', 'CAN', true, (SELECT id FROM provincia WHERE codigo = 'CAN')),
...
('D�leg', 'DELE', true, (SELECT id FROM provincia WHERE codigo = 'CAN')),
('Suscal', 'SUSC', true, (SELECT id FROM provincia WHERE codigo = 'CAN'))
ON CONFLICT DO NOTHING;

-- CARCHI
INSERT INTO canton (nombre, codigo, estado, provincia_id) VALUES
('Tulc�n', 'TULC', true, (SELECT id FROM provincia WHERE codigo = 'CAR')),
('Mont�far', 'MONT', true, (SELECT id FROM provincia WHERE codigo = 'CAR')),
...
('Bol�var', 'BOLI', true, (SELECT id FROM provincia WHERE codigo = 'CAR'))
ON CONFLICT DO NOTHING;

-- CHIMBORAZO
INSERT INTO canton (nombre, codigo, estado, provincia_id) VALUES
('Riobamba', 'RIOB', true, (SELECT id FROM provincia WHERE codigo = 'CHI')),
('Alaus�', 'ALAU', true, (SELECT id FROM provincia WHERE codigo = 'CHI')),
('Colta', 'COLT', true, (SELECT id FROM provincia WHERE codigo = 'CHI')),
('Cumand�', 'CUMA', true, (SELECT id FROM provincia WHERE codigo = 'CHI')),
...
('La Man�', 'LMAN', true, (SELECT id FROM provincia WHERE codigo = 'COT')),
('Pangua', 'PANG', true, (SELECT id FROM provincia WHERE codigo = 'COT')),
('Pujil�', 'PUJI', true, (SELECT id FROM provincia WHERE codigo = 'COT')),
('Salcedo', 'SALC', true, (SELECT id FROM provincia WHERE codigo = 'COT')),
('Saquisil�', 'SAQU', true, (SELECT id FROM provincia WHERE codigo = 'COT')),
...
('Marcabel�', 'MARC', true, (SELECT id FROM provincia WHERE codigo = 'ORE')),
('Pasaje', 'PASA', true, (SELECT id FROM provincia WHERE codigo = 'ORE')),
('Pi�as', 'PINAS', true, (SELECT id FROM provincia WHERE codigo = 'ORE')),
...
('Quinind�', 'QUIN', true, (SELECT id FROM provincia WHERE codigo = 'ESM')),
...
-- GALAPAGOS
INSERT INTO canton (nombre, codigo, estado, provincia_id) VALUES
('San Crist�bal', 'SCRI', true, (SELECT id FROM provincia WHERE codigo = 'GAL')),
...
('Dur�n', 'DURA', true, (SELECT id FROM provincia WHERE codigo = 'GUA')),
...
('Samborond�n', 'SAMB', true, (SELECT id FROM provincia WHERE codigo = 'GUA')),
('Santa Luc�a', 'SLUC', true, (SELECT id FROM provincia WHERE codigo = 'GUA')),
('Sim�n Bol�var', 'SBOL', true, (SELECT id FROM provincia WHERE codigo = 'GUA')),
...
('Marcelo Maridue�a', 'MMAR', true, (SELECT id FROM provincia WHERE codigo = 'GUA'))
ON CONFLICT DO NOTHING;
...
('San Miguel de Urcuqu�', 'SMUR', true, (SELECT id FROM provincia WHERE codigo = 'IMB'))
ON CONFLICT DO NOTHING;
...
('Esp�ndola', 'ESPI', true, (SELECT id FROM provincia WHERE codigo = 'LOJ')),
('Gonzanam�', 'GONZ', true, (SELECT id FROM provincia WHERE codigo = 'LOJ')),
('Macar�', 'MACA', true, (SELECT id FROM provincia WHERE codigo = 'LOJ')),
...
('V�nces', 'VINC', true, (SELECT id FROM provincia WHERE codigo = 'LRI')),
...
('Bol�var', 'BOLI', true, (SELECT id FROM provincia WHERE codigo = 'MAN')),
...
('Puerto L�pez', 'PLOP', true, (SELECT id FROM provincia WHERE codigo = 'MAN')),
('Jama', 'JAMA', true, (SELECT id FROM provincia WHERE codigo = 'MAN')),
('Jaramij�', 'JARJ', true, (SELECT id FROM provincia WHERE codigo = 'MAN')),
...
('Suc�a', 'SUCU', true, (SELECT id FROM provincia WHERE codigo = 'MSA')),
...
('Logro�o', 'LOGR', true, (SELECT id FROM provincia WHERE codigo = 'MSA')),
...
('Mej�a', 'MEJI', true, (SELECT id FROM provincia WHERE codigo = 'PIC')),
('Pedro Moncayo', 'PMON', true, (SELECT id FROM provincia WHERE codigo = 'PIC')),
('Rumi�ahui', 'RUMI', true, (SELECT id FROM provincia WHERE codigo = 'PIC')),
...
('Sucumb�os', 'SUCU', true, (SELECT id FROM provincia WHERE codigo = 'SUC'))
ON CONFLICT DO NOTHING;

-- TUNGURAHUA
INSERT INTO canton (nombre, codigo, estado, provincia_id) VALUES
('Ambato', 'AMBA', true, (SELECT id FROM provincia WHERE codigo = 'TUN')),
('Ba�os de Agua Santa', 'BANO', true, (SELECT id FROM provincia WHERE codigo = 'TUN')),
...
('Santiago de P�llaro', 'SDPI', true, (SELECT id FROM provincia WHERE codigo = 'TUN')),
...
('Centinela del C�ndor', 'CCON', true, (SELECT id FROM provincia WHERE codigo = 'ZCH')),
('Palanda', 'PALA', true, (SELECT id FROM provincia WHERE codigo = 'ZCH'))
ON CONFLICT DO NOTHING;

-- =====================================================
-- 5. INSERCIѓN DE USUARIOS (solo si no existen)
-- =====================================================
...
(false, 0, NOW(), '0987654322', true, 'jefe', 'Jefe Ventas', 'jefe@test.com', 'Mar�a', 'Quito, Ecuador', 'admin123'),
...
-- =====================================================
-- 5.5. MIGRACIѓN DE IMсGENES (url_imagen → arma_imagen)
-- =====================================================
...
SELECT 'Categor�as de armas:' as info, COUNT(*) as total FROM categoria_arma;
...
SELECT 'La base de datos est� lista para usar con el frontend.' as mensaje;

-- ========================================
-- DATOS ADICIONALES PARA DESARROLLO
-- ========================================
-- Agregados autom�ticamente para tener un ambiente de desarrollo completo
...
INSERT INTO usuario (
    ...
    'karolina.pazmino', 'Pazmi�o', 'karritogeova@hotmail.com', 
...
-- ========================================
-- MIGRACI�N DE IM�GENES A TABLA arma_imagen
-- ========================================
...
-- ========================================
-- CRITICO: Sin esto, los IDs saltan despu�s de resets
