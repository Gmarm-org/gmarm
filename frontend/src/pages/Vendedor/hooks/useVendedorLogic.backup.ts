...
export const useVendedorLogic = () => {
  ...
  const exportarClientesAExcel = useCallback(async () => {
    try {
      ...
      // Obtener TODOS los clientes del vendedor (sin paginación)
      if (!user || !user.id) {
      ...
      for (const cliente of todosLosClientes) {
        try {
          ...
          // Cast a any para acceder a todas las propiedades que vienen de la API
          const clienteData = cliente as any;
          
          // Obtener tipo de cliente completo
          const tipoClienteNombre = clienteData.tipoProcesoNombre || clienteData.tipoClienteNombre || clienteData.tipoCliente || 'N/A';
          
          // Preparar datos para Excel
          const fila = {
            'CI/RUC': cliente.numeroIdentificacion || '',
            'Nombres': cliente.nombres || '',
            'Apellidos': cliente.apellidos || '',
            'Nombre Completo': `${cliente.nombres || ''} ${cliente.apellidos || ''}`.trim(),
            'Tipo Cliente': tipoClienteNombre,
            'Email': cliente.email || '',
            'Teléfono Principal': cliente.telefonoPrincipal || '',
            'Teléfono Secundario': clienteData.telefonoSecundario || '',
            'Dirección': clienteData.direccion || '',
            'Provincia': clienteData.provincia || '',
            'Cantón': clienteData.canton || '',
            'Fecha Nacimiento': clienteData.fechaNacimiento ? new Date(clienteData.fechaNacimiento).toLocaleDateString('es-EC') : '',
            'Estado Militar': clienteData.estadoMilitar || '',
            'Código ISSFA': clienteData.codigoIssfa || '',
            'Rango': clienteData.rango || '',
            'Representante Legal': clienteData.representanteLegal || '',
            'RUC Empresa': clienteData.ruc || '',
            'Nombre Empresa': clienteData.nombreEmpresa || '',
            'Dirección Fiscal': clienteData.direccionFiscal || '',
            'Teléfono Referencia': clienteData.telefonoReferencia || '',
            'Correo Empresa': clienteData.correoEmpresa || '',
            'Provincia Empresa': clienteData.provinciaEmpresa || '',
            'Cantón Empresa': clienteData.cantonEmpresa || '',
            'Estado Cliente': cliente.estado || '',
            'Fecha Creación': clienteData.fechaCreacion ? new Date(clienteData.fechaCreacion).toLocaleDateString('es-EC') : '',
            'Vendedor': user.nombres && user.apellidos ? `${user.nombres} ${user.apellidos}` : user.email || '',
            // Información del arma
            'Arma Nombre': arma ? arma.armaNombre || '' : '',
            'Arma Código': arma ? arma.armaCodigo || '' : '',
            'Arma Modelo': arma ? arma.armaModelo || '' : '',
            'Arma Calibre': arma ? arma.armaCalibre || '' : '',
            'Arma Descripción': arma ? `${arma.armaNombre || ''} ${arma.armaModelo || ''} ${arma.armaCalibre || ''}`.trim() : '',
            'Número de Serie': arma ? arma.numeroSerie || '' : '',
            'Estado Arma': arma ? arma.estado || '' : '',
            'Cantidad': arma ? arma.cantidad || '' : '',
            'Precio Unitario': arma ? parseFloat(arma.precioUnitario || '0').toFixed(2) : '',
            'Precio Total': arma ? (parseFloat(arma.precioUnitario || '0') * parseInt(arma.cantidad || '1')).toFixed(2) : '',
          };
          
          datosExportacion.push(fila);
        } catch (error) {
          console.warn(`⚠️ Error cargando armas para cliente ${cliente.id}:`, error);
          // Agregar cliente sin información de arma
          const clienteData = cliente as any;
          const tipoClienteNombre = clienteData.tipoProcesoNombre || clienteData.tipoClienteNombre || clienteData.tipoCliente || 'N/A';
      ...
      const columnWidths = [
        ...
        { wch: 15 }, // Estado Militar
        { wch: 15 }, // Código ISSFA
        { wch: 15 }, // Rango
        { wch: 30 }, // Representante Legal
        { wch: 15 }, // RUC Empresa
        { wch: 30 }, // Nombre Empresa
        { wch: 40 }, // Dirección Fiscal
        { wch: 15 }, // Teléfono Referencia
        { wch: 30 }, // Correo Empresa
        { wch: 20 }, // Provincia Empresa
        { wch: 20 }, // Cantón Empresa
        { wch: 20 }, // Estado Cliente
