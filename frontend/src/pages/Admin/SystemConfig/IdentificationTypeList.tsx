...
const IdentificationTypeList: React.FC = () => {
  ...
  const handleDelete = async (identificationType: IdentificationType) => {
    if (window.confirm(`¿Desactivar el tipo de identificación "${identificationType.nombre}"? No se eliminará de la base de datos, solo cambiará su estado a inactivo para mantener auditoría.`)) {
      try {
        // No eliminar, solo cambiar estado a false (inactivo)
        await identificationTypeApi.update(identificationType.id, { ...identificationType, estado: false });
        await loadIdentificationTypes();
        alert('Tipo de identificación desactivado exitosamente');
      } catch (error) {
        console.error('Error desactivando tipo de identificación:', error);
        alert('Error al desactivar el tipo de identificación');
      }
    }
  };
