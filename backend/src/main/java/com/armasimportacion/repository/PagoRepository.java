...
public interface PagoRepository extends JpaRepository<Pago, Long> {
    ...
    @Query("SELECT p FROM Pago p WHERE p.cliente.id = :clienteId ORDER BY p.fechaCreacion DESC")
    List<Pago> findUltimosPagosByCliente(@Param("clienteId") Long clienteId, Pageable pageable);
    
    @Query("SELECT SUM(p.montoPagado) FROM Pago p WHERE p.cliente.id = :clienteId")
    BigDecimal findTotalPagadoByCliente(@Param("clienteId") Long clienteId);
    
    @Query("SELECT SUM(p.montoPendiente) FROM Pago p WHERE p.cliente.id = :clienteId")
    ...
    @Query("SELECT p FROM Pago p WHERE p.cliente.id = :clienteId AND p.estado = :estado ORDER BY p.fechaCreacion DESC")
