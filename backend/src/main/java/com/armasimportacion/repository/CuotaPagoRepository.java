...
public interface CuotaPagoRepository extends JpaRepository<CuotaPago, Long> {
    ...
    @Query("SELECT c FROM CuotaPago c WHERE c.pago.cliente.id = :clienteId AND c.estado = :estado")
    List<CuotaPago> findByClienteIdAndEstado(@Param("clienteId") Long clienteId, @Param("estado") EstadoCuotaPago estado);
    
    @Query("SELECT c FROM CuotaPago c WHERE c.pago.cliente.id = :clienteId ORDER BY c.fechaVencimiento ASC")
